                    ...
                    <div class="dropdownMenu flex flex-col text-sm absolute top-2 bottom-2 right-2 hidden border border-gray-600 w-sm bg-[var(--h-secondary-bg-color)] text-[var(--text-color)] shadow-xl rounded-2xl opacity-0 transition-all duration-300 ease-in-out z-[100] p-4">
                        ...
                        <div class="grow overflow-y-auto my-scrollbar-2 p-1">
                            <div class="grid grid-cols-1 gap-4">
                                @foreach ($search_fields as $search_field => $value)
                                    ...
                                    @else
                                        <x-input label="{{ $search_field }}" id="{{ $value['id'] }}" type="{{ $value['type'] }}" :dataClearable="true" dataFilterPath="{{ $value['dataFilterPath'] }}" value="{{ $value['value'] ?? '' }}"/>
                                    @endif
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-2 p-1">
                            <button type="button" onclick="showAllData()"
                                title="Clear filters and load all records"
                                class="col-span-2 px-4 py-2.5 bg-[var(--secondary-bg-color)] border border-gray-600 text-[var(--text-color)] font-medium rounded-lg hover:bg-[var(--h-bg-color)] transition-all duration-300 ease-in-out cursor-pointer hover:scale-[0.98]">
                                All Data
                            </button>
                            <button type="button" onclick="clearAllSearchFields()"
                                title="Clear filters (Alt+C)"
                                class="px-4 py-2.5 bg-[var(--bg-error)] border border-[var(--bg-error)] text-[var(--text-error)] font-medium rounded-lg hover:bg-[var(--h-bg-error)] transition-all duration-300 ease-in-out cursor-pointer hover:scale-[0.98]">
                                Clear
                            </button>
                            <button type="button" onclick="applyFilters()"
                                title="Apply filters (Alt+S)"
                                class="px-4 py-2.5 bg-[var(--primary-color)] border border-[var(--primary-color)] text-[var(--text-color)] font-medium rounded-lg hover:bg-[var(--h-primary-color)] transition-all duration-300 ease-in-out cursor-pointer hover:scale-[0.98]">
                                Apply Filters
                            </button>
                        </div>
                    </div>
