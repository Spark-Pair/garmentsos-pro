@extends('app')
@section('title', 'Edit Setup | ' . $client_company->name)
@section('content')
    <div class="max-w-lg mx-auto">
        <x-search-header heading="Edit Setup" link linkText="Show Setups" linkHref="{{ route('setups.index') }}"/>
    </div>

    <form id="edit-setups-form" action="{{ route('setups.update', ['setup' => $setup->id]) }}" method="post"
        ...
        @csrf
        @method('PUT')
        <x-form-title-bar title="Edit Setup" />
        ...
        <div id="step1" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-1 gap-4">
                ...
                <x-input
                    ...
                    value="{{ old('short_title', $setup->short_title) }}"
                />
    ...
    <script>
        (() => {
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const titleInput = document.querySelector('[data-setup-title]');
                const shortTitleInput = document.querySelector('[data-setup-short-title]');
                const form = document.getElementById('edit-setups-form');
